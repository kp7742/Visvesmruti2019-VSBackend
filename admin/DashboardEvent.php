...
if ($admin[0]["Department"] == "All" || $admin[0]["EventID"] == null) {
    header("Location: index.php");
    exit();
}
$event = getEventByID($admin[0]["EventID"]);
$evententries = getEventEntryByE($event[0]["EVID"]);
$eventprice = $event[0]["EVPrice"];
...
foreach ($evententries as $entry) {
    if ($event[0]["isTeamEvent"] == 1) {
        if ($entry["isTeamLeader"] == 1) {
            ...
            $totalprice = ($event[0]["isSinglePrice"] == 1) ? $eventprice : ($eventprice * count($teamentries));
            if ($entry["isPaid"] == 1) {
...
<head>
    ...
    <script type="text/javascript">const filename = "VSData-<?php echo $admin[0]["Department"] . "-" . $event[0]["EVName"];?>"</script>
</head>
            ...
            <div class="col-lg-7 col-md-5 col-sm-0 col-xs-12">
                <div class="header-top-menu">
                    <ul class="nav navbar-nav mai-top-nav">
                        ...
                        <?php if ($admin[0]["Department"] == "Computer") {
                            echo '<li class="nav-item"><a href="EventRounds.php" class="nav-link">Event Rounds</a></li>';
                        } ?>
                    </ul>
                </div>
            </div>
                        ...
                        <li class="nav-item">
                            <a href="#" data-toggle="dropdown" role="button" aria-expanded="false"
                               class="nav-link dropdown-toggle">
                                <span class="admin-name"><?php echo $admin[0]['Name'] . "</br>" . $event[0]["EVName"]; ?></span>
                                <span class="angle-down-topmenu"><i class="fa fa-angle-down"></i></span>
                            </a>
                            <ul role="menu" class="dropdown-header-top author-log dropdown-menu animated flipInX">
                                <li><a href="DashboardEvent.php?Logout">Log Out</a></li>
                            </ul>
                        </li>
                    </ul>
                </div>
                                ...
                                if ($event[0]["isTeamEvent"] == 1) {
                                    foreach ($teamarr as $team) {
                                        foreach ($team as $entry) {
                                            ...
                                            echo "<tr>" .
                                                ...
                                                '<td>' . ($entry["isTeamLeader"] == 1 ? 'Yes' : '') . '</td>' .
                                                "</tr>";
                                        }
                                        echo "<tr>" .
                                            "<td>-</td>" .
                                    ...
                                    foreach ($evententries as $entry) {
                                        ...
                                        echo "<tr>" .
                                            ...
                                            ($entry["isPaid"] == 1 ? 'fa fa-check' : 'fa fa-times') . '"></i></td>' .
                                            "<td>-</td>" .
                                            "</tr>";
                                    }
                                }
